...
namespace DevExpress.DevAV.Common.Utils {
    public class DeviceDetector {
        ...
        private static string[] dellModel = new[] { "Venue 8 Pro 5830" };
        private static KnonwnHardwareKind[] dellModelKind = new[] { KnonwnHardwareKind.DellPro8 };
        private static void ParseKindDell(HardwareInfo res) { ParseKindCore(res, dellModel, dellModelKind); }

        private static bool ParseKindCore(HardwareInfo res, string[] model, KnonwnHardwareKind[] kind) {
            var i = Array.IndexOf<string>(model, res.Model);
            if(i < 0) return false;
            res.Kind = kind[i];
            return true;
        }

        private static string[] msModel = new[] { "Surface with Windows 8 Pro", "Surface Pro 2", "Surface Pro 3" };
        private static KnonwnHardwareKind[] msModelKind = new[] { KnonwnHardwareKind.SurfacePro, KnonwnHardwareKind.SurfacePro2, KnonwnHardwareKind.SurfacePro3 };
        private static void ParseKindMicrosoft(HardwareInfo res) { ParseKindCore(res, msModel, msModelKind); }
