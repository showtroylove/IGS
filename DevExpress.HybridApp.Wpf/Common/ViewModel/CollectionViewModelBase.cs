...
namespace DevExpress.DevAV.Common.ViewModel
{
    ...
    {
        private EntitiesChangeTracker<TPrimaryKey> ChangeTrackerWithKey => (EntitiesChangeTracker<TPrimaryKey>)ChangeTracker;
        private readonly Action<TEntity> newEntityInitializer;
        private IRepository<TEntity, TPrimaryKey> Repository => (IRepository<TEntity, TPrimaryKey>)ReadOnlyRepository;
        ...
        /// Since CollectionViewModelBase is a POCO view model, and the instance of this class will also expose the NewCommand property that can be used as a binding source in views.
        /// </summary>
        public virtual void New() => GetDocumentManagerService().ShowNewEntityDocument(this, newEntityInitializer);

        /// <summary>
        /// Creates and shows a document that contains a single object view model for the existing entity.
        /// Since CollectionViewModelBase is a POCO view model, and the instance of this class will also expose the EditCommand property that can be used as a binding source in views.
        /// </summary>
        ...
        protected virtual IDocumentManagerService GetDocumentManagerService() => this.GetService<IDocumentManagerService>();

        protected virtual void OnBeforeEntityDeleted(TPrimaryKey primaryKey, TEntity entity) { }

        protected virtual void OnEntityDeleted(TPrimaryKey primaryKey, TEntity entity) => Messenger.Default.Send(new EntityMessage<TEntity, TPrimaryKey>(primaryKey, EntityMessageType.Deleted));
        ...
        private TProjection FindLocalProjectionWithSameKey(TProjection projectionEntity) {
            var primaryKeyAvailable = projectionEntity != null && Repository.ProjectionHasPrimaryKey(projectionEntity);
        ...
        protected virtual void OnEntitySaved(TPrimaryKey primaryKey, TEntity entity) => Messenger.Default.Send(new EntityMessage<TEntity, TPrimaryKey>(primaryKey, EntityMessageType.Changed));
        ...
        protected void DestroyDocument(IDocument document) => document?.Close();

        protected IRepository<TEntity, TPrimaryKey> CreateRepository() => (IRepository<TEntity, TPrimaryKey>)CreateReadOnlyRepository();

        protected override IEntitiesChangeTracker CreateEntitiesChangeTracker() => new EntitiesChangeTracker<TPrimaryKey>(this);

        private void VerifyProjectionType() 
        {
            var primaryKeyPropertyName = CreateRepository().GetPrimaryKeyPropertyName();
            if(TypeDescriptor.GetProperties(typeof(TProjection))[primaryKeyPropertyName] == null)
                throw new ArgumentException(
                    $"Projection type {typeof(TProjection).Name} should have primary key property {primaryKeyPropertyName}", "TProjection");
        }
        ...
        private void RegisterSelectEntityMessage() 
        {
            if(!ignoreSelectEntityMessage)
                Messenger.Default.Register<SelectEntityMessage>(this, OnSelectEntityMessage);
        }
