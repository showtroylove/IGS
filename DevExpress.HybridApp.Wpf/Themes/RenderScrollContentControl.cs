...
namespace DevExpress.DevAV.Themes {
    ...
    public class RenderScrollContentControl : ContentControl {
        ...
        static RenderScrollContentControl() {
            var ownerType = typeof(RenderScrollContentControl);
        ...
        private static void RaiseHorizontalScrollBarVisibilityChanged(DependencyObject d, DependencyPropertyChangedEventArgs e) {
            ((RenderScrollContentControl)d).RaiseHorizontalScrollBarVisibilityChanged(e);
        }

        private static void RaiseVerticalScrollBarVisibilityChanged(DependencyObject d, DependencyPropertyChangedEventArgs e) {
            ((RenderScrollContentControl)d).RaiseVerticalScrollBarVisibilityChanged(e);
        }

        private static void RaiseComputedHorizontalScrollBarVisibilityChanged(DependencyObject d, DependencyPropertyChangedEventArgs e) {
            ((RenderScrollContentControl)d).RaiseComputedHorizontalScrollBarVisibilityChanged(e);
        }

        private static void RaiseComputedVerticalScrollBarVisibilityChanged(DependencyObject d, DependencyPropertyChangedEventArgs e) {
            ((RenderScrollContentControl)d).RaiseComputedVerticalScrollBarVisibilityChanged(e);
        }

        private static void RaiseHorizontalValueChanged(DependencyObject d, DependencyPropertyChangedEventArgs e) {
            ((RenderScrollContentControl)d).RaiseHorizontalValueChanged(e);
        }

        private static void RaiseVerticalValueChanged(DependencyObject d, DependencyPropertyChangedEventArgs e) {
        ...
        public event EventHandler ComputedHorizontalScrollBarVisibilityChanged;
        public event EventHandler ComputedVerticalScrollBarVisibilityChanged;
        public void ScrollToHorizontalOffset(double x) {
            var child = Child;
            if(child == null) return;
            var childSize = GetChildSize(child, RenderSize);
            var childWidth = Math.Max(childSize.Width, RenderSize.Width);
            var h = CalculateValueFromArrange(childWidth, RenderSize.Width, x, HorizontalScrollBarVisibility);
            if(h < 0.0)
                h = 0.0;
        ...
        public void ScrollToVerticalOffset(double y) {
            ...
            if(child == null) return;
            var childSize = GetChildSize(child, RenderSize);
            var childHeight = Math.Max(childSize.Height, RenderSize.Height);
            var v = CalculateValueFromArrange(childHeight, RenderSize.Height, y, VerticalScrollBarVisibility);
            if(v < 0.0)
                v = 0.0;
        ...
        protected override Size MeasureOverride(Size availableSize) {
            ...
            var width = HorizontalScrollBarVisibility == ScrollBarVisibility.Disabled ? availableSize.Width : double.PositiveInfinity;
            var height = VerticalScrollBarVisibility == ScrollBarVisibility.Disabled ? availableSize.Height : double.PositiveInfinity;
            child.Measure(new Size(width, height));
            var childSize = GetChildSize(child, availableSize);
        ...
        protected override Size ArrangeOverride(Size finalSize) {
            ...
            var childSize = GetChildSize(child, finalSize);
            ...
            UpdateScroll(finalSize);
            return finalSize;
        }

        private void UpdateScroll(Size finalSize) {
            var child = Child;
            if(child == null) return;
            var childSize = GetChildSize(child, finalSize);
            childSize = new Size(Math.Max(childSize.Width, finalSize.Width), Math.Max(childSize.Height, finalSize.Height));
            ...
            double maximum;
            double viewportSize;
            var x = CalculateArrange(childSize.Width, finalSize.Width, HorizontalValue, HorizontalScrollBarVisibility, out enableScrolling, out computedScrollBarVisibility, out maximum, out viewportSize);
            ...
            var y = CalculateArrange(childSize.Height, finalSize.Height, VerticalValue, VerticalScrollBarVisibility, out enableScrolling, out computedScrollBarVisibility, out maximum, out viewportSize);
        ...
        private void UpdateOffsets(double x, double y) {
        ...
        private UIElement Child => VisualTreeHelper.GetChildrenCount(this) < 1 ? null : VisualTreeHelper.GetChild(this, 0) as UIElement;

        private static Size GetChildSize(UIElement child, Size availableSize) {
            var desiredSize = child.DesiredSize;
            var contentPresenter = child as ContentPresenter;
            var scalePanel = contentPresenter == null ? child as ISupportLayout : contentPresenter.Content as ISupportLayout;
            var perfectSize = scalePanel == null ? desiredSize : scalePanel.PerfectSize;
            var width = perfectSize.Width > availableSize.Width ? desiredSize.Width : perfectSize.Width;
            var height = perfectSize.Height > availableSize.Height ? desiredSize.Height : perfectSize.Height;
            return new Size(width, height);
        }

        private double CalculateArrange(double regularSize, double finalSize, double value, ScrollBarVisibility scrollBarVisibility, out bool enableScrolling, out Visibility computedScrollBarVisibility, out double maximum, out double viewportSize) {
            double arrange;
            if(scrollBarVisibility == ScrollBarVisibility.Disabled) {
                ...
                var k = Range / (regularSize - finalSize);
        ...
        private double CalculateValueFromArrange(double regularSize, double finalSize, double arrange, ScrollBarVisibility scrollBarVisibility) {
            if(scrollBarVisibility == ScrollBarVisibility.Disabled) return 0.0;
            var k = Range / (regularSize - finalSize);
            var viewportSize = k * finalSize;
            var enableScrolling = !double.IsNaN(viewportSize) && !double.IsInfinity(viewportSize) && viewportSize > 0.0;
            return enableScrolling ? arrange * k : 0.0;
        }

        private Visibility ComputeScrollBarVisibility(ScrollBarVisibility visibility, bool enableScrolling) {
            if(visibility == ScrollBarVisibility.Disabled || visibility == ScrollBarVisibility.Hidden) return System.Windows.Visibility.Collapsed;
            if(visibility == ScrollBarVisibility.Visible) return System.Windows.Visibility.Visible;
            return enableScrolling ? System.Windows.Visibility.Visible : System.Windows.Visibility.Collapsed;
        }

        private void RaiseHorizontalScrollBarVisibilityChanged(DependencyPropertyChangedEventArgs e) { InvalidateMeasure(); }
        private void RaiseVerticalScrollBarVisibilityChanged(DependencyPropertyChangedEventArgs e) { InvalidateMeasure(); }

        private void RaiseHorizontalValueChanged(DependencyPropertyChangedEventArgs e) {
            var child = Child;
            if(child == null) return;
            ...
            UpdateScroll(RenderSize);
        }

        private void RaiseVerticalValueChanged(DependencyPropertyChangedEventArgs e) {
            var child = Child;
            if(child == null) return;
            var newValue = (double)e.NewValue;
            var childSize = GetChildSize(child, RenderSize);
        ...
        private void RaiseComputedHorizontalScrollBarVisibilityChanged(DependencyPropertyChangedEventArgs e) { UpdateComputedBothScrollBarsVisiblity(); if(ComputedHorizontalScrollBarVisibilityChanged != null) ComputedHorizontalScrollBarVisibilityChanged(this, EventArgs.Empty); }
        private void RaiseComputedVerticalScrollBarVisibilityChanged(DependencyPropertyChangedEventArgs e) { UpdateComputedBothScrollBarsVisiblity(); if(ComputedVerticalScrollBarVisibilityChanged != null) ComputedVerticalScrollBarVisibilityChanged(this, EventArgs.Empty); }

        private void UpdateComputedBothScrollBarsVisiblity() {
