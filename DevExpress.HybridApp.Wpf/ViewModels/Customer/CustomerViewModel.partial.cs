...
namespace DevExpress.DevAV.ViewModels {
    partial class CustomerViewModel {
        ...
        public IEnumerable<object> UpdateMapItems(DateTime start, DateTime end) {
            ...
            if(items.Count() > 0) {
                var minimumSalesValue = items.Min<IGrouping<CustomerStore, Order>>(orders => orders.CustomSum(order => order.TotalAmount));
                var maximumSalesValue = items.Max<IGrouping<CustomerStore, Order>>(orders => orders.CustomSum(order => order.TotalAmount));
                var dif = maximumSalesValue - minimumSalesValue;
